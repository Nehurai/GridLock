...
feature_cols=['day_index','hour','minute','minutes_since_midnight','quarter','dayofweek','weekend','rush_hour','night_flag','hour_sin','hour_cos',
              ...
              'gh_mean','ts_mean','gh_ts_mean','road_mean','weather_mean','gh_hour_mean']
cat_cols=['geohash','geohash_prefix4','geohash_prefix5','RoadType','LargeVehicles','Landmarks','Weather','timestamp']

cb = CatBoostRegressor(iterations=200,learning_rate=0.05,depth=6,loss_function='RMSE',random_seed=42,verbose=False,thread_count=-1,allow_writing_files=False)
...
if len(corr_valid)>0:
    corr_model = CatBoostRegressor(iterations=150,learning_rate=0.06,depth=6,loss_function='RMSE',random_seed=42,verbose=False,thread_count=-1,allow_writing_files=False)
